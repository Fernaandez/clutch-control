<template>
    <AppLayout :current-moto-id="motorcycle.id">
        <div class="w-full max-w-full min-w-0 overflow-x-hidden box-border px-4 py-6 pb-24">
            <div class="mb-6 flex items-start gap-3 w-full min-w-0">
                ...
                <div class="min-w-0 flex-1 pt-0.5">
                    <h1 class="text-2xl font-bold text-purple-400 break-words">{{ $t('upgrades_history.title') }}</h1>
                ...
                <div v-for="log in history" :key="log.id" class="bg-brand-surface rounded-xl p-4 border-l-4 border-purple-500/50 shadow-lg relative">
                    <div class="flex justify-between items-start gap-3">
                        ...
                        <div class="text-right flex-shrink-0">
                            ...
                            <button @click="openShowModal(log)" class="mt-2 inline-flex items-center justify-center w-8 h-8 rounded-full bg-brand-dark border border-purple-500/40 text-purple-400 hover:bg-purple-500 hover:text-white transition" title="Veure">
        ...
        <div v-if="selectedLog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-purple-500/30 rounded-xl p-6 w-full max-w-sm max-h-[min(90vh,36rem)] overflow-y-auto overscroll-contain shadow-[0_0_20px_rgba(168,85,247,0.1)]">
                <button @click="selectedLog = null" class="absolute top-4 right-4 inline-flex items-center justify-center w-8 h-8 rounded-full bg-brand-dark border border-purple-500/50 text-purple-400 hover:bg-purple-500 hover:text-white transition" aria-label="Tancar">
        ...
        <div v-if="selectedPhoto" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/95">
            <button @click="selectedPhoto = null" class="absolute top-4 right-4 inline-flex items-center justify-center w-10 h-10 rounded-full bg-brand-dark border border-purple-500/50 text-purple-400 hover:bg-purple-500 hover:text-white transition" aria-label="Tancar">
            ...
            <img :src="$page.props.storageUrl + '/' + selectedPhoto" alt="Foto mod" class="max-h-[85vh] max-w-full object-contain rounded-lg border border-purple-900/30">
        </div>
    </AppLayout>
</template>

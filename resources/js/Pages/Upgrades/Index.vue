    ...
    <AppLayout :current-moto-id="motorcycle.id">
        <div class="px-4 py-6 pb-24">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div class="flex-1">
                    <h1 class="text-2xl font-bold text-purple-400">{{ $t('upgrades.title') }}</h1>
                    <p class="text-brand-muted text-sm">{{ motorcycle.brand }} {{ motorcycle.model }}</p>
                </div>
                <div class="flex gap-2">
                    <Link :href="route('motorcycles.upgrades.history', motorcycle.id)" class="bg-brand-black border border-brand-dark text-gray-300 px-4 py-2 rounded-lg font-bold text-sm hover:border-purple-500 hover:text-purple-400 transition flex items-center gap-2">{{ $t('upgrades.history_link') }}</Link>
                    <button @click="showCreateModal = true" class="bg-purple-600 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-lg hover:bg-purple-500 transition">{{ $t('upgrades.new_upgrade') }}</button>
                </div>
            </div>
        ...
        <div v-if="showShowModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-purple-500/30 rounded-xl p-6 w-full max-w-sm shadow-[0_0_20px_rgba(168,85,247,0.1)]">

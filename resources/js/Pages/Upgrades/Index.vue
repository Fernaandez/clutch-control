<template>
    <AppLayout :current-moto-id="motorcycle.id">
            ...
            <div class="mb-6 space-y-3 w-full min-w-0">
                <div class="flex items-start gap-3 min-w-0">
                    ...
                    <div class="min-w-0 flex-1 pt-0.5">
                        <h1 class="text-2xl font-bold text-purple-400 break-words">{{ $t('upgrades.title') }}</h1>
                        <p class="text-brand-muted text-sm truncate">{{ motorcycle.brand }} {{ motorcycle.model }}</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2 w-full">
                    <Link :href="route('motorcycles.upgrades.history', motorcycle.id)" class="flex-1 min-w-[calc(50%-0.25rem)] sm:flex-initial sm:min-w-0 inline-flex justify-center items-center bg-brand-black border border-brand-dark text-gray-300 px-4 py-2.5 rounded-lg font-bold text-sm hover:border-purple-500 hover:text-purple-400 transition">{{ $t('upgrades.history_link') }}</Link>
                    <button type="button" @click="showCreateModal = true" class="flex-1 min-w-[calc(50%-0.25rem)] sm:flex-initial sm:min-w-0 bg-purple-600 text-white px-4 py-2.5 rounded-lg font-bold text-sm shadow-lg hover:bg-purple-500 transition">{{ $t('upgrades.new_upgrade') }}</button>
                </div>
            </div>
            ...
            <div v-else class="space-y-4">
                <div v-for="task in tasks" :key="task.id" class="bg-brand-surface rounded-xl p-5 border-l-4 border-purple-500 shadow-lg relative overflow-hidden group">
                    ...
                    <div class="flex justify-end gap-3 relative z-10 mt-4">
                        ...
                        <button @click="openCompleteModal(task)" class="bg-brand-dark border border-purple-500/30 text-purple-400 hover:bg-purple-500 hover:text-white px-3 py-1 rounded text-sm font-bold transition flex items-center gap-1 active:scale-95 shadow-lg">{{ $t('upgrades.installed') }}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-purple-900 rounded-xl p-6 w-full max-w-sm max-h-[min(90vh,32rem)] overflow-y-auto overscroll-contain shadow-lg">
                <h3 class="text-xl font-bold text-white mb-4">{{ $t('upgrades.new_upgrade_title') }}</h3>
                <form @submit.prevent="submitCreate">
                    <div class="space-y-4">
                        <div>
                            <label class="text-gray-400 text-sm">{{ $t('upgrades.what_upgrade') }}</label>
                        ...
                        <div>
                            <label class="text-gray-400 text-sm">{{ $t('upgrades.details') }}</label>
                    ...
                    <button type="submit" class="mt-6 w-full bg-purple-600 text-white font-bold py-2 rounded hover:bg-purple-500 transition">{{ $t('upgrades.save') }}</button>
                </form>
            </div>
        </div>
        ...
        <div v-if="showCompleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-purple-500 rounded-xl p-6 w-full max-w-sm max-h-[min(90vh,36rem)] overflow-y-auto overscroll-contain shadow-lg">
                <h3 class="text-xl font-bold text-white mb-1">{{ $t('upgrades.upgrade_done_title') }}</h3>
                <p class="text-sm text-brand-muted mb-4">{{ $t('upgrades.task_label') }} <span class="text-purple-400">{{ selectedTask?.title }}</span></p>
                <form @submit.prevent="submitComplete">
                    <div class="space-y-4">
                        <div>
                            <label class="text-gray-400 text-sm">{{ $t('upgrades.date') }}</label>
                        ...
                        <div>
                            <label class="text-gray-400 text-sm">{{ $t('upgrades.km_current') }}</label>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-gray-400 text-sm">{{ $t('upgrades.price') }}</label>
                            ...
                            <div>
                                <label class="text-gray-400 text-sm">{{ $t('upgrades.workshop') }}</label>
                        ...
                        <div>
                            <label class="text-gray-400 text-sm">{{ $t('upgrades.invoice_photo') }}</label>
                    ...
                    <button type="submit" class="mt-6 w-full bg-purple-600 text-white font-bold py-3 rounded-lg hover:bg-purple-500 transition shadow-lg">{{ $t('upgrades.confirm') }}</button>
                </form>
            </div>
        </div>
        ...
        <div v-if="showShowModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-purple-500/30 rounded-xl p-6 w-full max-w-sm max-h-[min(90vh,32rem)] overflow-y-auto overscroll-contain shadow-[0_0_20px_rgba(168,85,247,0.1)]">
                ...
                <p class="text-xs text-purple-400/70 uppercase tracking-widest mb-4">{{ $t('upgrades.title') }}</p>
                <div class="space-y-3">

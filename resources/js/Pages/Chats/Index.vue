                ...
                <Link v-for="chat in conversations" :key="chat.id" :href="route('chats.show', chat.id)" 
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="relative min-w-[3rem]">
                            ...
                            <!-- Group -->
                            <template v-else>
                                <img v-if="chatPhotoUrl(chat)" :src="chatPhotoUrl(chat)" class="w-12 h-12 rounded-full object-cover border-2 border-brand-neon/30">
                                <div v-else class="w-12 h-12 rounded-full bg-gradient-to-br from-brand-neon/40 to-brand-dark flex items-center justify-center text-white font-black text-xl border-2 border-brand-neon/30">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-2 flex flex-wrap gap-1">
                                ...
                                <span v-if="chat.motorcycle" class="inline-flex items-center gap-1.5 bg-brand-dark/50 border border-brand-neon/20 text-brand-neon px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest">
                                    <AppIcon name="moto" size="xs" />
                                    {{ chat.motorcycle.brand }} {{ chat.motorcycle.model }}
                                </span>
                                <span v-if="chat.event" class="inline-flex items-center gap-1.5 bg-brand-dark/50 border border-brand-neon/20 text-brand-neon px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest">
...
const chatPhotoUrl = (chat) => {
    if (!chat?.photo) return null;
    if (chat.photo.startsWith('http')) return chat.photo;
    return storageUrl + '/' + chat.photo;
};

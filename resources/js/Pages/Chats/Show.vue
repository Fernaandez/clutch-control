<template>
    <AppLayout :title="pageTitle" :hide-bottom-nav="true">
        <div class="relative">
            ...
            <!-- CAPÇALERA DEL XAT -->
            <div class="fixed left-3 right-3 z-[40] bg-brand-surface border border-brand-dark rounded-2xl px-4 py-3 flex items-center justify-between shadow-[0_4px_10px_rgba(0,0,0,0.3)] transition-all safe-horizontal" style="top: var(--app-header-total-height);">
            ...
            <div class="pt-[90px] pb-3 px-5 space-y-1 h-[calc(100vh-var(--app-header-total-height)-var(--safe-bottom)-5.5rem)] overflow-y-auto flex flex-col safe-horizontal" ref="messagesContainer">
                <template v-for="(msg, idx) in localMessages" :key="msg.id">
                    ...
                    <div class="flex flex-col max-w-[85%] mb-4"
            ...
            <!-- CAIXA D'ENVIAMENT -->
            <div class="fixed left-0 right-0 z-[40] bg-brand-surface border-t border-brand-dark px-3 pt-3 transition-all safe-horizontal" style="bottom: 0; padding-bottom: calc(var(--safe-bottom) + 0.5rem);">
                <form @submit.prevent="submit" class="flex gap-2 max-w-[720px] mx-auto">

    ...
    <AppLayout :current-moto-id="motorcycle.id">
        <div class="max-w-xl mx-auto px-6 py-6 pb-24 cc-fade-in">
            ...
            <header class="flex items-center gap-3 mb-6">
                ...
                <h1 class="cc-title flex-1 truncate">{{ $t('repairs.title') }}</h1>
                <div class="flex items-center gap-2 flex-shrink-0">
                    <button type="button" @click="showCreateModal = true" class="cc-icon-btn" :aria-label="$t('repairs.new_repair')">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
            ...
            <div v-if="tasks.length" class="divide-y divide-white/[0.06]">
                ...
                >
                    ...
                    <div class="flex items-center gap-1 flex-shrink-0">
                        ...
                        <button type="button" @click="deleteTask(task)" class="cc-icon-btn text-gray-500 hover:text-red-400" :title="$t('repairs.delete_confirm')">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            ...
            <div v-else class="flex flex-col items-center justify-center text-center py-16 px-6">
                <p class="text-base font-semibold text-gray-300">{{ $t('repairs.no_repairs') }}</p>
                <p v-if="$t('repairs.no_repairs_hint')" class="mt-1 text-sm text-gray-500 max-w-xs">{{ $t('repairs.no_repairs_hint') }}</p>
                <div class="mt-6">
            ...
            <nav class="mt-12 pt-8 border-t border-white/[0.06]">
                <Link :href="route('motorcycles.repairs.history', motorcycle.id)" class="cc-btn-text">
                    {{ $t('repairs.history_link') }}
                </Link>
            </nav>
        </div>
        ...
        <div v-if="showCreateModal" class="fixed inset-0 z-[4000] flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-white/[0.08] rounded-2xl p-6 max-w-sm w-full max-h-[min(90vh,32rem)] overflow-y-auto overscroll-contain">
                <h3 class="text-lg font-medium text-white mb-6">{{ $t('repairs.new_repair_title') }}</h3>
                <form @submit.prevent="submitCreate">
                    <div class="space-y-5">
                        <div>
                            <label class="cc-section-label">{{ $t('repairs.what_broke') }}</label>
                        ...
                        <div>
                            <label class="cc-section-label">{{ $t('repairs.details') }}</label>
        ...
        <div v-if="showCompleteModal" class="fixed inset-0 z-[4000] flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-white/[0.08] rounded-2xl p-6 max-w-sm w-full max-h-[min(90vh,36rem)] overflow-y-auto overscroll-contain">
                <h3 class="text-lg font-medium text-white mb-1">{{ $t('repairs.repair_done_title') }}</h3>
                <p class="text-sm text-gray-500 mb-6">{{ $t('repairs.task_label') }} {{ selectedTask?.title }}</p>
                <form @submit.prevent="submitComplete">
                    <div class="space-y-5">
                        <div>
                            <label class="cc-section-label">{{ $t('repairs.date') }}</label>
                            <input v-model="completeForm.date" type="date" class="w-full mt-2" required>
                        </div>
                        <div>
                            <label class="cc-section-label">{{ $t('repairs.km_current') }}</label>
                            <input v-model="completeForm.km_at_moment" type="number" class="w-full mt-2" required>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="cc-section-label">{{ $t('repairs.price') }}</label>
                                <input v-model="completeForm.cost" type="number" step="0.01" class="w-full mt-2" required>
                            </div>
                            <div>
                                <label class="cc-section-label">{{ $t('repairs.workshop') }}</label>
                                <input v-model="completeForm.description" type="text" class="w-full mt-2" required>
                            </div>
                        </div>
                        <div>
                            <label class="cc-section-label">{{ $t('repairs.invoice_photo') }}</label>
        ...
        <div v-if="showShowModal" class="fixed inset-0 z-[4000] flex items-center justify-center p-4">
            ...
            <div class="relative bg-brand-surface border border-white/[0.08] rounded-2xl p-6 max-w-sm w-full max-h-[min(90vh,32rem)] overflow-y-auto overscroll-contain">
                ...
                <p class="text-sm text-gray-500 mb-6">{{ $t('repairs.title') }}</p>
                <div>
                    <p class="cc-section-label">{{ $t('repairs.details') }}</p>

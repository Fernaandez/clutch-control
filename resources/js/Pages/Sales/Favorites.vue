<template>
    <AppLayout :title="$t('sales.favorites')">
        <div class="px-6 pt-10 pb-28 max-w-xl mx-auto cc-fade-in">
            <p class="text-sm text-gray-500">{{ $t('sales.favorites') }}</p>
            <p class="mt-3 text-[64px] leading-[0.9] font-light tracking-[-0.04em] text-white tabular-nums">
                {{ sales.length }}
            </p>
            <p class="mt-2 text-sm text-gray-500">{{ $t('sales.tab_favorites') }}</p>
            ...
            <div v-if="sales.length" class="divide-y divide-white/[0.06] mt-10">
                <div v-for="sale in sales" :key="sale.id" class="flex items-center gap-3 py-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p
                            v-if="sale.state === 'venuda' || sale.state === 'reservat'"
                            class="mt-1 text-xs"
                    ...
                    <Link :href="route('sales.show', sale.id) + '?from=fav'" class="cc-btn-text flex-shrink-0">
                        {{ $t('common.view') }}
                    </Link>
                </div>
            </div>
            ...
            <div v-else class="py-16 text-center mt-10">
                <p class="text-base font-semibold text-gray-300">{{ $t('sales.no_favorites_short') }}</p>
                <Link :href="route('sales.index')" class="cc-btn-text mt-4 inline-flex">
                    {{ $t('sales.marketplace_title') }}
                </Link>
            </div>
        </div>
    </AppLayout>

<template>
    <AppLayout :title="`Historial ${sale.motorcycle?.brand}`">
        <div class="px-4 py-6 pb-24 max-w-4xl mx-auto">
            ...
            <div class="bg-blue-900/20 border border-blue-500/30 p-4 rounded-xl mb-6 shadow-inner text-sm flex gap-3 text-blue-200">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8 flex-shrink-0 text-blue-400"><path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" /></svg>
                <div>
                    <strong>Transparència Activada.</strong> Aquest venedor ha decidit mostrar els registres de manteniment, reparacions i millores fetes en aquesta moto utilitzant l'aplicació per poder acreditar el seu estat.
                </div>
            </div>
            ...
            <div v-if="showFilters" class="bg-brand-black border border-brand-dark rounded-xl p-4 mb-6 shadow-inner space-y-4 animate-fade-in-down">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.search') }}</label>
                    ...
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.type') }}</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.date_start') }}</label>
                    ...
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.date_end') }}</label>
                    ...
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.km_min') }}</label>
                    ...
                    <div>
                        <label class="text-[10px] text-gray-500 uppercase font-bold">{{ $t('maintenance.km_max') }}</label>
                ...
                <div class="border-t border-brand-dark pt-3 mt-2">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button @click="toggleSortDir" class="bg-brand-surface border border-brand-dark px-3 rounded-lg text-white hover:border-brand-neon transition font-bold text-xs">
                            {{ filters.sortDir === 'desc' ? $t('common.desc') : $t('common.asc') }}
                        </button>
                    </div>
                </div>
                
                <button @click="resetFilters" class="w-full text-xs text-red-500 font-bold uppercase tracking-widest hover:text-white mt-2 p-2 bg-red-900/20 border border-red-500/20 rounded-lg">
            ...
            <div v-else class="space-y-4">
                <div v-for="log in filteredHistory" :key="log.id" class="bg-brand-surface rounded-xl p-4 border border-brand-dark shadow-lg relative flex flex-col sm:flex-row justify-between gap-4 group hover:border-brand-neon transition animate-fade-in">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs text-gray-400 font-mono tracking-tight">{{ formatDate(log.date) }}</span>
                        </div>
                    ...
                    <div class="sm:text-right min-w-[100px] flex sm:flex-col items-center justify-between sm:justify-center border-t sm:border-t-0 sm:border-l border-brand-dark pt-3 sm:pt-0 sm:pl-4 mt-2 sm:mt-0">
                        <p class="text-xl font-bold font-mono text-brand-neon w-full">{{ log.cost }} €</p>
                        <p class="text-xs text-gray-500 font-black uppercase tracking-widest">{{ log.km_at_moment }} km</p>
                    </div>
...
const toggleSortDir = () => { filters.value.sortDir = filters.value.sortDir === 'desc' ? 'asc' : 'desc'; };

const resetFilters = () => {
    filters.value = {
        search: '', type: 'all', dateStart: '', dateEnd: '', priceMin: '', priceMax: '', kmMin: '', kmMax: '', sortBy: 'date', sortDir: 'desc'
    };
};

const formatDate = (dateString) => { return new Date(dateString).toLocaleDateString(locale.value); };
...
const filteredHistory = computed(() => {
    ...
    if (filters.value.dateEnd) { result = result.filter(log => log.date <= filters.value.dateEnd); }
    if (filters.value.priceMin !== '') { result = result.filter(log => parseFloat(log.cost) >= filters.value.priceMin); }
    if (filters.value.priceMax !== '') { result = result.filter(log => parseFloat(log.cost) <= filters.value.priceMax); }
    if (filters.value.kmMin !== '') { result = result.filter(log => log.km_at_moment >= filters.value.kmMin); }
    if (filters.value.kmMax !== '') { result = result.filter(log => log.km_at_moment <= filters.value.kmMax); }
...
const totalFilteredCost = computed(() => {
    return filteredHistory.value.reduce((sum, log) => sum + parseFloat(log.cost || 0), 0);
});
</script>

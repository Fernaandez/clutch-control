<template>
    ...
    <div class="min-h-screen flex flex-col justify-center items-center bg-brand-black px-6 py-12">
        ...
        <div class="w-full max-w-md bg-brand-surface border border-brand-dark p-8 rounded-2xl shadow-2xl relative overflow-hidden">
            ...
            <h2 class="text-2xl font-black text-white uppercase tracking-tight mb-2 text-center mt-4">{{ $t('reset_password.title') }}</h2>
            <p class="text-gray-400 text-xs text-center mb-6">{{ $t('reset_password.subtitle') }}</p>

            <form @submit.prevent="submit" class="space-y-5">
                
                <div>
                    <label class="block font-bold text-xs uppercase tracking-wider text-gray-400 mb-2 ml-1" for="email">{{ $t('reset_password.email') }}</label>
                    <input 
                        id="email" 
                        type="email" 
                ...
                <div>
                    <label class="block font-bold text-xs uppercase tracking-wider text-gray-400 mb-2 ml-1" for="password">{{ $t('reset_password.new_password') }}</label>
                    <input 
                        id="password" 
                        type="password" 
                ...
                <div>
                    <label class="block font-bold text-xs uppercase tracking-wider text-gray-400 mb-2 ml-1" for="password_confirmation">{{ $t('reset_password.confirm_password') }}</label>
                    <input 

...
const loginWithGoogle = async () => {
    ...
    if (Capacitor.isNativePlatform()) {
        // Native app: use in-app browser so the session cookie is shared
        const { Browser } = await import('@capacitor/browser');
        
        await Browser.open({ 
            url: googleUrl,
            windowName: '_self'  // opens inside the app's webview context
        });
        
        // When browser closes, reload to pick up the new session

        ...
        <div class="p-4 space-y-4">
            ...
            <div class="pt-2">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <Link :href="route('motorcycles.maintenance.index', moto.id)" class="col-span-2 flex items-center gap-3 bg-brand-surface border border-brand-dark rounded-xl p-4 hover:border-brand-neon hover:bg-brand-dark/50 transition group text-left">
                    ...
                    <Link :href="route('motorcycles.repairs.index', moto.id)" class="flex items-center gap-3 bg-brand-surface border border-brand-dark rounded-xl p-4 hover:border-red-500 hover:bg-brand-dark/50 transition group text-left">
                        <div class="bg-red-500/20 p-2 rounded-lg text-red-400 group-hover:text-red-300">
                ...
                <div class="p-6">
                    <div class="flex flex-col items-end font-mono text-xl sm:text-2xl text-gray-300 mb-6 bg-brand-black/50 p-4 rounded-lg border border-brand-dark">
                        <div class="mb-1 text-gray-500 flex items-center gap-2">
                             <span class="text-xs uppercase tracking-widest font-sans mr-2">Abans:</span>{{ parseFloat(moto.current_km).toFixed(1) }}
                        </div>
                        <div class="flex items-center gap-2 text-brand-neon font-bold w-full justify-end">
                            <span class="text-2xl">+</span>
                            <input ref="kmInputRef" v-model="addKmForm.km_to_add" type="number" step="0.1" placeholder="0" class="w-32 bg-transparent border-b-2 border-brand-neon text-right focus:ring-0 focus:border-white text-white font-bold p-1 text-2xl placeholder-brand-dark" @keyup.enter="submitKm">
                        </div>
                        <div class="w-full h-0.5 bg-gray-500 my-2"></div>
                        <div class="text-white font-black flex items-center gap-2">
                             <span class="text-xs uppercase tracking-widest font-sans text-brand-muted mr-2">Total:</span>{{ (parseFloat(moto.current_km) + (parseFloat(addKmForm.km_to_add) || 0)).toFixed(1) }}
                        </div>
                    </div>

...
// Sense contrasenya (compte només de Google) es confirma escrivint el correu.
// La condició ha de ser exactament la mateixa que fa servir ProfileController.
const needsEmailConfirmation = computed(() => !usePage().props.auth.user?.has_password);
        ...
        <Modal :show="confirmingUserDeletion" @close="closeModal">
            <div class="p-6 bg-brand-black border border-white/[0.08]">
                ...
                <p class="mt-2 text-sm text-gray-400">
                    {{ needsEmailConfirmation ? $t('profile.delete_confirm_hint_google') : $t('profile.delete_confirm_hint') }}
                </p>

                <div v-if="needsEmailConfirmation" class="mt-6">
                    <label class="sr-only">{{ $t('profile.email_placeholder') }}</label>
                    <input
                        ref="confirmInput"
                    ...
                    />
                    <p v-if="form.errors.confirm_email" class="text-red-400 text-xs mt-1">{{ form.errors.confirm_email }}</p>
                </div>

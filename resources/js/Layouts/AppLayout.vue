<template>
    <div class="min-h-screen bg-brand-black text-gray-200 font-sans relative" style="padding-bottom: calc(6.5rem + env(safe-area-inset-bottom));"> 
        ...
        <nav class="bg-brand-surface border-b border-brand-dark sticky top-0 z-[3000] px-4 flex items-center justify-between shadow-lg relative" style="padding-top: env(safe-area-inset-top); height: calc(3.5rem + env(safe-area-inset-top));">
            ...
            <div class="relative min-w-[1.75rem] flex justify-end group">
                <button class="text-[10px] font-bold text-gray-400 hover:text-white uppercase transition px-2 py-1 border border-gray-600 rounded">
                    {{ $i18n.locale }}
                </button>
                <div class="absolute top-full right-0 mt-2 bg-brand-surface border border-brand-dark rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col overflow-hidden">
                    <button @click="changeLanguage('es')" :class="{'bg-brand-dark text-brand-neon': $i18n.locale === 'es'}" class="text-xs px-4 py-2 hover:bg-brand-dark text-left">ES</button>
                    <button @click="changeLanguage('ca')" :class="{'bg-brand-dark text-brand-neon': $i18n.locale === 'ca'}" class="text-xs px-4 py-2 hover:bg-brand-dark text-left">CA</button>
                    <button @click="changeLanguage('en')" :class="{'bg-brand-dark text-brand-neon': $i18n.locale === 'en'}" class="text-xs px-4 py-2 hover:bg-brand-dark text-left">EN</button>
                </div>
            </div>
        </nav>
        ...
        <nav class="fixed bottom-[-2px] left-0 right-0 w-full bg-brand-surface border-t border-b-2 border-brand-surface border-t-brand-dark flex justify-around items-center z-[3000] shadow-[0_-4px_10px_rgba(0,0,0,0.5)]" style="padding-bottom: calc(0.75rem + env(safe-area-inset-bottom)); height: calc(4.75rem + env(safe-area-inset-bottom));">
            ...
            <Link 
                ...
                :class="{'text-brand-neon drop-shadow-[0_0_5px_rgba(12,225,181,0.6)]': route().current('profile.edit')}" 
                class="flex flex-col items-center justify-center gap-0.5 text-gray-500 hover:text-gray-300 transition w-16 h-full cursor-pointer"
            >
                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
